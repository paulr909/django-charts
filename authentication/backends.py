...
class ActiveSessionAuthentication(authentication.BaseAuthentication):
    auth_error_message = {"success": False, "msg": "User not logged in"}
    ...
    def _authenticate_credentials(self, token):
        ...
        try:
            ...
            msg = {"success": False, "msg": "No user found with this token"}
            raise exceptions.AuthenticationFailed(msg)
        if not user.is_active:
            msg = {"success": False, "msg": "User has been deactivated"}
            raise exceptions.AuthenticationFailed(msg)
